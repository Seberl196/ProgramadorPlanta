...
def mostrar_pagina_nueva_ot() -> None:
    ...
    st.caption("Las nuevas OT se guardan sin tren asignado hasta que sean programadas.")
    ...
    for orden in ordenes:
        with st.container(border=True):
            columna_info, columna_tren, columna_accion = st.columns([2, 2, 1])

            with columna_info:
                st.write(f"**OT {orden['numero_ot']}**")
                st.write(f"Duración: {orden['duracion_horas']:.2f} h")
